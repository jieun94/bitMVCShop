...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr class="ct_list_pop">
		...
		<td align="left">
		<% if( request.getParameter("menu").equals("manage") ) { %>
			<a href="/getProduct.do?productNo=<%=prodVO.getProdNo() %>&menu=manage"><%= prodVO.getProdName() %></a>
		<% } else { %>
			<% if(prodVO.getProTranCode().contains("0")) { %>
				<a href="/getProduct.do?productNo=<%=prodVO.getProdNo() %>&menu=search"><%= prodVO.getProdName() %></a>
			<% } else { %>
				<%= prodVO.getProdName() %>
			<% } %>
		<% } %>
		...
		<% for(int i=1;i<=totalPage;i++){ %>
			<% if( request.getParameter("menu").equals("manage") ) { %>
				<% if(request.getParameter("searchKeyword")!=null) { %>
					<a href="/listProduct.do?page=<%=i%>&menu=manage&searchKeyword=<%= searchVO.getSearchKeyword()%>"><%=i %></a>
				<% } else{ %>
